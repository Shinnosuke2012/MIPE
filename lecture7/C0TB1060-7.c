...
int main(){
  ...
  for(j=0 ; j<n ; j++){ // 擬似乱数(-25.≦prn＜25.)の作成
    prn = 2.*25.*(double)rand()/(RAND_MAX) - 25.;
    i = (int)(((prn+25.)/50.)*9.);  // 擬似乱数の値(-25.≦prn≦25.)を配列の添字（0≦i≦9）に変換
  ...
  for(i=-5 ; i<=5 ; i++){ // 擬似乱数の分布(-2.5から2.5までを0.5刻みで表示する)
    ca = (double)i/2.; // 階級下限
    cb = (double)(i+1)/2.; // 階級上限
...
// 計算結果をgnuplotで画面に表示し，pngファイルに出力する．
  ...
  fprintf(pipe,"set xrange [-2.55:2.55]\n");  // x軸の範囲
  ...
  fprintf(pipe,"set output \"C0TB1060-7.png\"\n");  //pngファイル名
  fprintf(pipe,"replot \n");  // 再描画
  fflush(pipe);
